...
class TestGetToplevelPath:
    ...
    def test_get_toplevel_path(self, repo: Path) -> None:
        assert get_toplevel_path() == str(repo)

    def test_get_toplevel_path_with_no_repo(self, tmp_path: Path) -> None:
...
def test_get_all_refs(repo: Path) -> None:
    ...
    refs = list(get_all_refs(str(repo)))
    assert len(refs) == 3
...
class TestGetRefs:
    ...
    def test_get_refs(self, repo: Path) -> None:
        subprocess.check_call(["git", "commit", "--allow-empty", "-m", "Initial commit"])
        subprocess.check_call(["git", "branch", "test-branch"])
        subprocess.check_call(["git", "tag", "test-tag"])
        ...
        refs = list(get_refs(str(repo), tag_whitelist, branch_whitelist, remote_whitelist))
        assert len(refs) == 2
        assert any(ref.name == "test-branch" and ref.source == "heads" for ref in refs)
        assert any(ref.name == "test-tag" and ref.source == "tags" for ref in refs)

    def test_get_refs_with_empty_repo(self, repo: Path) -> None:
        refs = list(get_refs(str(repo), [], [], []))
        assert len(refs) == 0


class TestFileExists:
    def test_file_exists(self, repo: Path) -> None:
        (repo / "test-file").write_text("test content")
        subprocess.check_call(["git", "add", "test-file"])
        subprocess.check_call(["git", "commit", "-m", "Add test file"])

        assert file_exists(str(repo), "master", "test-file")
        assert not file_exists(str(repo), "master", "nonexistent-file")

    def test_file_exists_with_invalid_ref(self, repo: Path) -> None:
        assert not file_exists(str(repo), "invalid-ref", "test-file")


class TestCopyTree:
    def test_copy_tree(self, repo: Path, tmp_path: Path) -> None:
        (repo / "file1").write_text("content1")
        (repo / "file2").write_text("content2")
        ...
        dst = (tmp_path / "dst")
        dst.mkdir()
        copy_tree(str(repo), str(dst), version_ref)

        assert (dst / "file1").read_text() == "content1"
        assert (dst / "file2").read_text() == "content2"

    def test_copy_tree_with_subdir(self, repo: Path, tmp_path: Path) -> None:
        sub = repo / "subdir"
        sub.mkdir()
        ...
        copy_tree(str(repo), str(dst), version_ref, sourcepath="subdir")

        assert (dst / "subdir" / "file1").read_text() == "content1"
        assert not (dst / "file2").exists()

    def test_copy_tree_with_symlink(self, repo: Path, tmp_path: Path) -> None:
        file = repo / "file"
        file.write_text("content")
        os.symlink(file, repo / "symlink")
        ...
        assert (dst / "symlink").readlink() == (repo / "file")
